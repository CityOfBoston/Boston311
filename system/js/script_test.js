...
// Added in July 2010 to add PDF icon to pdf links
////updated Sept 2010 to add Google Analytics tracking to page links
$(document).ready(function(){
	...
	$('a').each(function() {
		...
		if(linkurl.substr(linkurl.lastIndexOf('.') + 1) == 'pdf'){
			...
			if(link.children("img").length == 0){
				link.html(link.html() + '<img src="/Images_Documents/pdf.gif" height="12" width="12" style="margin-left: 7px; position: relative; top: 3px;background:transparent;">');
			}
		//is this an external link?
			...
			//if we wanted to do something with internal, non-pdf links, this would be the place to do it
		}
	})
}); 